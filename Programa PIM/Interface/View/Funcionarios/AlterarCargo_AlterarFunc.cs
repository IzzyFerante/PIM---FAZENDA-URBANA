...
{
    public partial class AlterarCargo_AlterarFunc : Form
    {
        private Cargo_AlterarFunc formPrincipal;

        public AlterarCargo_AlterarFunc(string cod, Cargo_AlterarFunc cargo)
        {
            InitializeComponent();
            InitializeTabOrder();

            string codCargo = cod.PadLeft(5, '0');
            txtCodCargo.Text = codCargo;
            formPrincipal = cargo;
        }
        ...
        private void bttConfirmar_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Confirmar alteração?", "Atenção", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                acoes.AtualizaDadosCARGO("cargo", "nome_cargo", txtDescCargo.Text, "cod_cargo", txtCodCargo.Text);  //  Alterando dado no banco
        ...
        private void AlterarCargo_AlterarFunc_Load(object sender, EventArgs e)
        {
            AcoesFuncionarios acoes = new AcoesFuncionarios();
